...
1. Explain the difference between `RDBMS` and `SQL`.
  ...
  `RDBMS`is Relational Database Management Systems, it is a type of database. `SQL` is a language used to communicate with it.

1. Why do tables need a `primary key`?

  A table has a `primary key` to uniquely identify each row, this way you can query that specific information.
...
1. What do we need in order to have a _many to many_ relationship between two tables.
  ...
  In a `many to many` relationship, you need a third table. This third table with have `one to many` relationships on both sides.
